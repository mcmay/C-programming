...
void getBcSn(char * s1, int len1, char * s2, int len2);
...
int main(void)
{
	...
	while(Loop)
	{
		switch(option)
		{
			case '1':
				getItem(&temp);
				found = findItem(list, temp.barCode, temp.shopName);
				if(found == NULL)
				{
					if(findRelatedItem(list, temp.barCode) == false)
				...
				break;
			case '2':
				getBcSn(bc, BCLEN, sn, SNLEN);
				found = findItem(list, bc, sn);
				if(found == NULL)
...
void getItem(Item * pItem)
{
	...
	if(ch == '`')
		setOption(&ch, '6');
	if(ch >= '1' && ch <= '6')
		strcpy(pItem->unit, units[ch-48-1]);
	else
	{
		char unit[UNITLEN];
...
void getBcSn(char * s1, const int len1, char * s2, const int len2)
{
